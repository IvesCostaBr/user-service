import json, importlib, inspect, os


class CacheManager:
    def __init__(self):
        self.instance = self.__load_instance(os.environ.get("CACHE_TYPE", "redis"))

    def __load_instance(self, cache_name):
        module = importlib.import_module(f"src.infra.cache.{cache_name}")
        classes = inspect.getmembers(module, inspect.isclass)
        found_manager = list(filter(lambda x: cache_name.capitalize() in x[0], classes))
